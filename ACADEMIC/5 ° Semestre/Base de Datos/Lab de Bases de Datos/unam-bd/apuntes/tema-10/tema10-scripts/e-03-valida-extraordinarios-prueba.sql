...
--validar que efectivamente el registro en estudiante_extraordinario existe.
declare
  v_count number;
begin
  select count(*) into v_count
  from estudiante_extraordinario
  where estudiante_id = 23
  and num_examen = 7
  and asignatura_id = 19;
  if v_count > 0 then
    dbms_output.put_line('OK, examen registrado');
  else
    raise_application_error(-20001,' ERROR: El registro no se insertó');
  end if;
end;
/
...
Prompt =======================================
Prompt Prueba 2 (negativa: La prueba es exitosa si el trigger lanza un Error)
...
begin
	...
	insert into estudiante_extraordinario(estudiante_id,num_examen,calificacion,
    asignatura_id)
  values(1,1,null,1);
	-- Si se llega a este punto, significa que el trigger no está funcionando, 
  --se lanza excepcion
