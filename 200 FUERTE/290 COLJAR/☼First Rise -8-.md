---
estado: En ejecución
status: Activo
fecha_creado: 2024-05-29
fecha_terminado:
terminado: false
tipo: ptetra
tipo_p: ☼Plan
tipo_nota: primaria
modelo:
  - "[[►Tetrapetal]]"
  - "[[►GEZZER]]"
sucesor: "[[☼Second Rise]]"
etapa: 8
Completado: 4
Incompleto: 5
Total: 9
tags:
  - "#Active_kanban/FR-4-"
nombre: ☼First_Rise
image: "[[🔅FIRST RISE -8-.jpg]]"
banner: "![[🔅FIRST RISE -8-.jpg]]"
fecha_entrega: 2025-01-25
tipo_misión: MISIÓN PRINCIPAL
---
# ***☼First Rise {-8-}***
>***El primer paso para ser FERSA***

| ⤵                                         | **‼**                                             |
| ----------------------------------------- | ------------------------------------------------- |
| [[Kanban - ☼First Rise\|Kanban]]          | [[‼Sanando el Alma - Consolidando el Capítulo 1]] |
| [[INTERCONN - SERIES (☼FR).excalidraw]]   |                                                   |
| **[[SCHEDULE -FR-5- 2-2.pdf\|SCHEDULE]]** |                                                   |
| [[Leading - FR -8-]]                      |                                                   |

# Descripción
La misión principal de la ***máquina formidable*** para construir una **disciplina firme**

---
# Objetivo
Mediante la ***Primer Asamblea***, se estableció en el punto A:
![[1° Asamblea#^PuntoA]]
![[1° Asamblea#^ObjetivoFR]]
es decir:
> [!important] Objetivo
> El **==objetivo del primer surgimiento==** sería inculcar una ==***disciplina naciente***==.
> Esto mediante un objetivo concreto:
>  - Conseguir el ***MES MÁQUINA*** [^3]


Para poder alzarme como ***La Máquina Formidable***, debo asegurar primero el **poder del humano**, la ***disciplina***, para poder tener la capacidad de manejar cualquier tarea, sea del tamaño que sea.

> [!NOTE] # Proyectos del ***Primer Surgimiento*** **5° Etapa**
> Para ello, debo demostrar que he aprendido de mis Errores y Fracasos venidos de [[♠PEFYC]]
> Por ende, mediante [[♠DesigntheFuture]], se presentan los siguientes 8 ptetras o similares para cumplir en este cuarto y último intento del primer surgimiento.
> ![[🔅FIRST RISE -8-.jpg]]

^311605

*Si no es ahora, ¿entonces cuando?*
>El primer surgimiento debe ser la primer misión de un Fersa, pues esta asegura en su objetivo el inculcar en su esencia la importancia de la disciplina, así como saber manejarla.

| *LIGHTVOLTS requeridos*              | *LIGHTVOLTS de egreso*                                        |
| ------------------------------------ | ------------------------------------------------------------- |
| Experiencias de [[♠PEFYC]]           | El poder humano de la ***disciplina***                        |
| [[♠Legacy]]: legados de mis mentores | Control total de la ***STAMINA***                             |
|                                      | Control del ***Fuerte*** y despliegue del modo ***INSANITY*** |
|                                      | Despliegue del ***[[MES MÁQUINA]]***                          |

---
# Fundamentación
Tras el tercer intento, ahora con más experiencias que antes y sabiendo cuáles fueron mis errores, me propongo a intentarlo de nuevo, teniendo **en cuenta aquello que me impidió avanzar y que voy a hacer para mejorar** [^1]
*Hasta que el objetivo no se haya cumplido, no podré avanzar en mi esencia*

># ***8° Etapa***
>Después de una larga jornada de *intentos no exitosos*[^4], todo culminará con el intento número 8, donde se demostrará de una manera arriesgada, si el portados es digno o no de llevar el manto de [[🌐La Máquina Formidable]]
>Esto dado que enfrentará el 3 año de la carrera, el cuál busca:
>![[♠A-T SUPER UNIVERSITARIO {-5-}#^0c6ace]]
>Un reto total para el portador, que además deberá buscar el balance con la 2° y 3° sección más que nunca.
>El portador deberá demostrar entre otras aptitudes como la de la ***disciplina***, si es capaz de poder *partir en una aventura por su cuenta de manera independiente* en el próximo semestre, así como preparar su alma para un capítulo donde *el corazón no es reprimido*, el *cuerpo florece* y la **disciplina le permite construir imperio**.

---
# PLANEACIÓN - MODELO
***Órganos Alimentadores***
Se conservan los [[♠Leading]] para los ptetras comunes
## ➡ [[♠Leading - ☼First Rise DEPRECATED|♠Leading ]]

## **Línea del SUPER UNIVERSITARIO**
### ▶ [[♠A-T SUPER UNIVERSITARIO]] como *Primer Plano*
- Legendarizar materias anteriores -> **FORMULARIUM**
#### ⏩ [[BRAINTHUNDER]]

### ⏩ [[Un Ingeniero de Verdad]]

### -▶ [[♣T - Programador Junior]]
- [[Programando diario por 1 hora hasta conseguir INTERNSHIP]]
- [[Entrenando 3 horas diarias hasta subir de clasificatoria en Codeforces]]

## **Línea de proyectos personales** (Proyectos Mayores como *Segundo Plano*)

### ▶ [[♣EL NUEVO CURSO WEB]]
- Concretar planeación

### ▶ [[♣BuildYourEmpire]]
#### - [[♣VTA]]
- Renovar modelo VTA

### ▶ [[♣THE CLOVERS SOCIETY]]
- Hacer el juego de trivia: [[PROBATRÓN - MARATÓN]]

### ▶ [[♠Read]]
- Primera y Segunda Etapa

### ▶ [[♣AUTODIDACTA master]]
- Aprender cosas por mi cuenta
#### [[♣Platzi Master]]
- Aprender sobre nuevas tecnologías y ser productivo


## *SERIES* para la alimentación del [[♦(L)Wissen-garden|jardín]]-> [[INTERCONN - SERIES (☼FR).excalidraw|Diagrama]]

---
# Misión
La disciplina según la [[1° Asamblea|Primera Asamblea]]
- **Perseverancia**
- **Puntualidad**
- ***Productividad***

La ***productividad*** se cumplirá con:
- [ ] Haber logrado las 8 submisiones #MILESTONE 

- [ ] Para la **Perseverancia**, se debe mantener una **[[STAMINA]]** de orden superior a 80% en una secuencia de 5 semanas seguidas #MILESTONE [^2] (**1 MES MÁQUINA**)

- [ ] Para la **Puntualidad**, ~~se debe llegar a tiempo todos los días del nuevo semestre a lo largo de~~ ~~10 semanas seguidas~~ ~~lo que queda del semestre~~ *Respetar el* **SCHEDULE** #MILESTONE y llegar puntual a todos los eventos en los que sea citado:
	- [-] Desayuno con Angie ❌ 2025-07-20
	- [x] Chamba del 11 de julio ✅ 2025-07-20
	- [x] Colorines Trolleo ✅ 2025-12-26


---
# Frecuencia
En todo momento está en ejecución.

Para definir los tiempos de las acciones de la [[#PLANEACIÓN - MODELO]] se recurre al $SCH$

🔼 Se establece el $SCH$ como la unión de todos los tiempos de las acciones individuales del MODELO[^5]

- [x] [[SCHEDULE - ☼First Rise {-4-}.pdf]] ✅ 2024-06-03
- [x] [[SCHEDULE - 🔆FR -5-.pdf]] ✅ 2025-07-29
	
- [x] [[SCHEDULE - FR -6- 2.pdf]] ✅ 2025-12-26




---
![[Matrices de priorización (☼FR)]]





---
---
# ***INTERCONN***
## Conexiones
[[Kanban - ☼First Rise]]
Notas que han de considerarse para la revisión de la actual, para tener contexto
## Recursos
*Apoyan al contenido*
Aspectos visibles de las notas secundarias
MATEEXTRA relacionado
Recursos para la MISION y OBJETIVO
Recursos para estudiar o revisar temas relacionado. -> Para seguir aprendiendo

## Referencias
Referencia a notas externas como fuente de información
Referencias externas de otra fuente de información


[^1]: [[3° Asamblea]]
[^2]: Es decir, lograr 2 ***MES MÁQUINA***

[^3]: [[MES MÁQUINA]]

[^4]: Empezando con la primera iteración de la misión en 2020, estando en el primer año de la prepa cuando todo el fuerte emergió y fue creado; así como sus renovaciones importantes, como la renovación -4- en 2024 y la renovación -5- a inicios de 2025

[^5]: [[SCHEDULE#Conjunto $SCH$|Conjunto SCH]]
