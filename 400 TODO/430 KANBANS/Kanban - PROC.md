---

tipo: Kanban
tipo_nota: alimentadora/secundaria
kanban-plugin: board
pertenece a: "[[♠Olympic Mind - PROC]]"
recurrencia:
  - semanal

---

## ‼Org

- [ ] Daily:
	- [ ] ADHOC
	- [ ] Problema 6G
	- [ ] Codeforces
	- [ ] CP4
	- [ ] ThemeCP
- [ ] # TEMAS SEMANAL
	- T1


## Incoming Problems

- [ ] Establecer orden en [[PLAN PARA NO ESTAR DESEMPLEADOS (REMONTADA)]] ^3d4ba1
- [ ] # W1
	- [ ] NumTheory
		- [ ] Primes
	- [ ] Combi
	- [ ] Graphs
	- [ ] Prefix Sums
- [ ] Daily
	- [ ] ADHOC
	- [ ] Problema 6G
	- [ ] Codeforces
	- [ ] CP4
	- [ ] ThemeCP
- [ ] Terminar el Miércoles de Propedéutico ([[♣CPCFI]]) [[Mapeo de Problemas]]
- [ ] Sesión de Lectura de Libros OMM
- [ ] Aprender y entender lo casos de uso de cada una de las estructuras en C++ para CP
- [ ] [[Mapeo de Kattis]] 📅 2025-09-29
- [ ] [[Mapeo UVa Online judge]] 📅 2025-09-29


## # Secuencia de *Problemas*<br>‼Yi

**Complete**
- [x] Saloneo ✅ 2024-12-05
- [x] Organizar sesiones ✅ 2025-04-29
- [x] Material para Flood Fill ✅ 2025-05-02
- [x] Daily: ✅ 2025-05-18
	- [x] ADHOC ✅ 2025-05-18
	- [ ] Problema 6G
	- [ ] Codeforces
	- [ ] CP4
	- [ ] ThemeCP
- [x] Daily: ✅ 2025-07-23
	- [x] ADHOC ✅ 2025-07-21
	- [ ] Problema 6G
	- [ ] Codeforces
	- [ ] CP4
	- [ ] ThemeCP
- [x] Daily: ✅ 2025-07-24
	- [x] ADHOC ✅ 2025-07-23
	- [ ] Problema 6G
	- [ ] Codeforces
	- [ ] CP4
	- [ ] ThemeCP


## **Propuesta de temas**

- [ ] Crear Notion
- [ ] DSU 📅 2025-10-03 ^2eefaa
- [ ] Combinatoria 📅 2025-10-01 ^e9a48d
- [ ] Corregir las sesiones de Aprendizaje atrasadas desde el incoming de Obsidian
- [ ] multisets, mutimaps, unordered
- [ ] [[GCD]]
- [ ] # Semana 1
	DS


## Problems



## # **#STORMED**<br>#Questions ❓

- [ ] permutations of set
- [ ] diferencia entre push_back emplace_back
- [ ] Cuando se usan los partial_sort
- [ ] lower_bound upper_bound


## For ***Perform***

- [ ] Hacer un CheatSheet de los métodos de todas las estructuras para CP


## Out

- [ ] Sesión de revisión de material de lectures
- [ ] Investigar sobre bits
- [ ] Organizar notas


## # Secuencia de Acciones<br>⛓ ZZERIAL

**Complete**
- [x] [[‼ Repaying]] @[[17-03-2025]] ✅ 2025-03-17
	Problema de la segunda semana propedéutica
- [x] Daily: ✅ 2025-04-28
	- [x] ADHOC ✅ 2025-04-28
	- [ ] Problema 6G
	- [ ] ThemeCP
	- [ ] CP4
- [x] Daily: ✅ 2025-04-29
	- [x] ADHOC ✅ 2025-04-29
	- [ ] Problema 6G
	- [ ] ThemeCP
	- [x] CP4 ✅ 2025-04-29




%% kanban:settings
```
{"kanban-plugin":"board","list-collapse":[false,null,true,false]}
```
%%